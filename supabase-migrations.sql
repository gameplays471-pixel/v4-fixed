-- ═════════════════════════════════════════════════════════════════════════════
-- GEMgym — Migration consolidada de segurança (Supabase SQL Editor)
-- Data: 2026-08-07
-- Baseado na auditoria de segurança (OWASP Top 10 2021 + ASVS + CWE)
--
-- Esta migration consolida TODAS as alterações SQL necessárias para
-- corrigir as vulnerabilidades identificadas. Execute direto no
-- Supabase SQL Editor (ou via psql/supabase CLI).
--
-- ⚠️  IMPORTANTE: auth.uid() retorna UUID, mas as colunas id/userId/ownerId
--     são TEXT (Prisma cuid()). Por isso usamos auth.uid()::text em todas
--     as policies para evitar "operator does not exist: uuid = text".
--
-- ORDEM DE EXECUÇÃO:
--   1. RLS + Policies (seção 1–5)
--   2. CHECK constraints (seção 6)
--   3. Índices de performance (seção 7)
-- ═════════════════════════════════════════════════════════════════════════════
...
-- ═════════════════════════════════════════════════════════════════════════════
-- #4 FIX (CRÍTICO): Sem RLS, qualquer cliente com a anon key pode
-- ler/modificar TODOS os dados de qualquer tabela.
...
-- ═════════════════════════════════════════════════════════════════════════════
-- auth.uid()::text porque id é TEXT (cuid), não UUID
...
-- Workout: dono lê/modifica; templates são visíveis para todos autenticados
...
-- WorkoutPlan / WorkoutPlanItem: dono ou admin (via service_role)
...
-- ═════════════════════════════════════════════════════════════════════════════
-- 6. CHECK CONSTRAINTS — Validação no banco (defesa em profundidade)
-- ═════════════════════════════════════════════════════════════════════════════
-- #28 FIX: Impede roles inválidos direto no banco (além da validação Zod)
-- Mesmo que o app tenha bug, o banco rejeita o dado.
...
BEGIN
  ...
  -- Sex CHECK constraint (nullable + valores válidos)
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'User_sex_check'
  ) THEN
    ALTER TABLE "User" ADD CONSTRAINT "User_sex_check"
      CHECK ("sex" IS NULL OR "sex" IN ('M', 'F', 'Outro'));
  END IF;

  -- Goal CHECK constraint (valores válidos)
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'User_goal_check'
  ) THEN
    ALTER TABLE "User" ADD CONSTRAINT "User_goal_check"
      CHECK ("goal" IS NULL OR "goal" IN ('Emagrecimento', 'Hipertrofia', 'Condicionamento', 'Saúde', 'Performance'));
  END IF;
END $$;

-- ═════════════════════════════════════════════════════════════════════════════
-- 7. ÍNDICES DE PERFORMANCE E SEGURANÇA
-- ═════════════════════════════════════════════════════════════════════════════
-- Índices compostos para queries frequentes de RLS (auth.uid()::text = userId)
-- Sem esses índices, cada policy faria seq scan na tabela.
...
CREATE INDEX IF NOT EXISTS "BodyWeightLog_userId_idx" ON "BodyWeightLog" ("userId");
CREATE INDEX IF NOT EXISTS "ProgressPhoto_userId_idx" ON "ProgressPhoto" ("userId");
CREATE INDEX IF NOT EXISTS "LiveWorkoutSession_userId_idx" ON "LiveWorkoutSession" ("userId");
...
-- Índice para cleanup de rate limit expirado
CREATE INDEX IF NOT EXISTS "RateLimitAttempt_key_createdAt_idx" ON "RateLimitAttempt" ("key", "createdAt");

-- Índice para audit log por tipo+id (admin panel)
CREATE INDEX IF NOT EXISTS "AuditLog_entityType_entityId_idx" ON "AuditLog" ("entityType", "entityId");

COMMIT;

-- ═════════════════════════════════════════════════════════════════════════════
-- PÓS-MIGRATION: Verificar
-- ═════════════════════════════════════════════════════════════════════════════
-- Confirma que RLS está ativo em todas as tabelas:
...
-- Confirma CHECK constraints:
-- SELECT conname, conrelid::regclass FROM pg_constraint
--   WHERE conname IN ('User_role_check', 'User_sex_check', 'User_goal_check');

-- Verifica contas com hash legado (não-bcrypt):
-- SELECT id, email, "passwordHash" FROM "User"
--   WHERE "passwordHash" NOT SIMILAR TO '\$2[aby]\$.*';
